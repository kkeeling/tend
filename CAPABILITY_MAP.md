...
| Verify an approved external action or cleanup | Choose the exact visible mutation, cleanup, or routine group | `action:verify` rereads owner, selected action, current artifact/evidence, digest, execution grant, and freshly observed provider identity; a mismatch refuses mutation and the receipt names its assurance level |
