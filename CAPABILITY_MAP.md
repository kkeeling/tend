...
| Know what needs attention now | Open **Now**; select a normal card, chat, edit, or use its exact CTA | `workspace:now` returns the same ordered owner pointers and `workspace:instruct` routes only to that owner feed |
| Confirm an inferred commitment | Use **Yes, add this commitment** or **No, reject this commitment** on its normal card | `commitment:candidate:confirm` accepts or rejects the exact candidate; implied or unvalidated candidates never auto-create |
| Correct a mistaken commitment merge | Ask from the selected canonical card; retain both source receipts while the correction runs | `commitment:candidate:split` moves one attributable signal to a new commitment; `commitment:candidate:relink` moves it to an existing one, with events on both sides |
| Reconcile matching evidence from another feed | Review the confirmation on the commitment's current owner card | Cross-feed matches never mutate silently; `commitment:candidate:confirm` links only after checking the expected commitment version |
| Move responsibility to another feed | Ask from the selected commitment and review the target | `commitment:rehome` moves the versioned owner card only when no queued, working, or approved-blocked work exists and no external mutation grant would be carried implicitly |
| Record that a commitment may be done or contradicted | Review the resurfaced owner card and retained source trail | `commitment:completion:evidence` maps typed evidence to pending, fulfilled, or reopened; `commitment:signal:change` preserves edited, deleted, retracted, or conflicting source history |
| Audit source blind spots | Open **Coverage** for identity, freshness, completeness, age, and remediation | `workspace:coverage`; configure with `source:profile:set`, then record every successful or failed attempt |
| Explain or correct priority | Open **Priority Ledger** for the active version, explanations, and proposals | `workspace:priority`, `priority:evaluate`, `priority:correction`, and exact `priority:proposal:approve`; rules change only after approval |
...
| Verify an approved external action or cleanup | Choose the exact visible mutation, cleanup, or routine group | `action:verify` rereads owner, selected action, current artifact/evidence, digest, execution grant, and freshly observed provider identity; verification freshness is checked again at execution, a mismatch refuses mutation, and the receipt names its assurance level |
| Collect evidence | Render the resulting cards | `source:record-run` requires an identity/completeness proof for profiled sources; `source:attempt:record` records failed exits without advancing checkpoints; `sweep:record-batch` records the judged batch separately |
